...
resource "google_project_iam_member" "hetzner_roles" {
  for_each = toset([
    "roles/storage.objectAdmin",
  ])
  project = google_service_account.hetzner.project
  role    = each.key
  member  = google_service_account.hetzner.member
}

resource "google_service_account_key" "hetzner" {
  service_account_id = google_service_account.hetzner.account_id
}

resource "github_actions_secret" "hetzner_gsa" {
  repository      = module.hetzner_repo.name
